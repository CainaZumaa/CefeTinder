...
export class ChatEntityMapper {
  ...
  static toConversationDomain(
    ...
    const typingUsers = new Set<string>(
      (metadata.typingUsers as string[] | undefined) || []
    );
  ...
  static toParticipantDomain(entity: ParticipantEntity): Participant {
    return {
      ...
      lastSeen: entity.last_seen
        ? Timestamp.create(entity.last_seen)
        : undefined,
    };
  }
  ...
  static toMessageStatistics(
    ...
    const sentMessages = messageEntities.filter(
      (m) => m.status === "sent"
    ).length;
    const deliveredMessages = messageEntities.filter(
      (m) => m.status === "delivered"
    ).length;
    const readMessages = messageEntities.filter(
      (m) => m.status === "read"
    ).length;
  ...
  static isValidMessageEntity(entity: any): boolean {
    ...
    if (entity.delivered_at && isNaN(new Date(entity.delivered_at).getTime()))
      return false;
    if (entity.read_at && isNaN(new Date(entity.read_at).getTime()))
      return false;
  ...
  static isValidParticipantEntity(entity: any): boolean {
    ...
    if (entity.last_seen && isNaN(new Date(entity.last_seen).getTime()))
      return false;
   ...
   */
  static createMockMessageEntity(
   ...
   */
  static createMockConversationEntity(
