...
export class ChatWebSocketController {
  ...
  private setupJoinHandler(socket: Socket): void {
    socket.on("chat:join", async (room: string, userData: { email: string }) => {
      try {
        // Validar dados
        const validation = this.mapper.validateJoinRequest(room, userData.email);
        if (!validation.valid) {
  ...
  private async processJoin(socket: Socket, room: string, user: IUser): Promise<void> {
  ...
  private broadcastMessage(socket: Socket, room: string, user: IUser, texto: string): void {
