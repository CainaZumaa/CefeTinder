...
export class Conversation {
  ...
  getMessages(
    ...
    if (before) {
      const beforeIndex = this._messages.findIndex((msg) =>
        msg.id.equals(before)
      );
      if (beforeIndex !== -1) {
  ...
  getUnreadCount(userId: string): number {
    return this._messages.filter(
      (message) => !message.isFromUser(userId) && message.status !== "read"
    ).length;
  }

  // Getters
  get roomId(): ChatRoomId {
    return this._roomId;
  }
  ...
  get typingUsers(): string[] {
    return Array.from(this._typingUsers);
  }
  get events(): any[] {
    return [...this._events];
  }
